...
function showCountdownStatus(message, heading = "COUNTDOWN TO KICKOFF") {
  ...
  $("#countdown-heading").textContent = heading;
...
function renderCountdown(targetMs) {
  ...
  if (remaining <= 0) {
    showCountdownStatus("GAME IN PROGRESS", "GAME DAY");
    return;
  }
  ...
  const status = $("#countdown-status");
  card.classList.remove("hidden");
  $("#countdown-heading").textContent = "COUNTDOWN TO KICKOFF";
  ...
  setCountdownValue("#countdown-days", days);
  setCountdownValue("#countdown-hours", hours);
  setCountdownValue("#countdown-minutes", minutes);
  setCountdownValue("#countdown-seconds", seconds);
}
...
function startCountdown(game) {
  ...
  // Epoch milliseconds are generated by normalize_schedule.py so the display
  // never has to guess the season year, daylight-saving offset, or time zone.
  const epochValue = Number(game.kickoff_epoch_ms);
  const isoValue = game.kickoff_iso ? Date.parse(game.kickoff_iso) : NaN;
  const targetMs = Number.isFinite(epochValue) && epochValue > 0 ? epochValue : isoValue;

  if (!Number.isFinite(targetMs)) {
    showCountdownStatus("KICKOFF TIME TBD");
    return;
  }

  renderCountdown(targetMs);
  countdownTimer = setInterval(() => renderCountdown(targetMs), 1000);
}
